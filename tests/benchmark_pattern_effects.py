...
def variants():
    ...
    modes += [("Pixelate",PixelateModifier(pixel_size=12)),
              ...
              ("Custom / native color",HalftoneModifier(dot_style="custom",custom_render_mode="original",
                  custom_svg='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path fill="#744bd4" d="M50 0L100 90H0Z"/><circle fill="#e9b94a" cx="50" cy="56" r="20"/></svg>'))]
    return modes
...
def semantic_checks(renderer):
    ...
    comparisons=[]
    for name,modifier in variants()[:11]:
        gpu=pixels(renderer.render(image,modifier)).astype(np.float32)/255
        cpu=pixels(apply_pattern_effect(image,modifier)).astype(np.float32)/255
...
def pipeline_timings(source,frames):
    ...
    canvas.chapter=ChapterDocument()
    ...
    try:
        for name,modifier in (("Square",HalftoneModifier()),
            ...
            result.append({"name":name,"median_ms":round(statistics.median(samples),3),
        ...
        return {"source_uploads":gpu.uploads,"timings":result}
    finally:
        gpu=getattr(canvas,"_gpu_pattern_renderer",None)
        if gpu is not None:
            gpu.close()
...
def contact_sheet(renderer,folder):
    ...
    for index,(name,modifier) in enumerate(modes):
        ...
        result=image if modifier is None else renderer.render(image,modifier)
        if result is None:
...
def main():
    ...
    try:
        ...
        for name,modifier in variants():
            start=time.perf_counter()
            result=renderer.render(source,modifier)
