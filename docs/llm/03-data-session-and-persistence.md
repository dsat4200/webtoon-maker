...
The window holds the active repository, `SeriesDocument`, `ChapterDocument`, dirty flag, autosave timing, loading guards, contextual ribbon state, hierarchy expansion/selection restoration state, hotkey chord/hold state, and timers for autosave, series preferences, workspace layout, and chord prefixes.
...
Selection, camera position, command history, active tool, gesture state, local text undo, and render caches are not written into project JSON or recovery autosaves. Switching chapters binds a new chapter/tile store, clears the command stack and most transient interaction/cache state, and normally resets the view.
...
- Future chapter or series schemas above version 12 are rejected without rewrite.
- Chapter load accepts older schema values, rebuilds typed objects, migrates legacy text alignment into free quads, normalizes current invariants, validates, and sets the in-memory schema to 12.
