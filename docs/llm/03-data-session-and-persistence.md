...
- schema version 16 and `document_kind` (`chapter` or `asset`).
...
A Blender view is a normal serialized object with type `blender_view`, but it
has no independent position, size, transform, scene path, mask escape, underlay,
or opacity. It derives its geometry from its direct closed bounded shape and is
always the last/background-most child. Chapter validation permits one per
non-page shape and rejects Blender views in asset documents.

Its optional `BlenderViewportState` stores a normalized view quaternion, focal
location, distance, perspective/orthographic/camera projection, lens, and
camera-view zoom/offset. All objects share one external Blender scene; only the
nearest selected shape context is attached to the live native viewport. These
view-state updates are persisted and autosaved without entering comic Undo.
...
- Blender-frame parent, uniqueness, fixed-background, and canonical-field rules;
...
The chapter `TextObject.font_size` field remains numeric in schema version 16. Fractional or out-of-range legacy document values continue rendering unchanged until a user edits that specific object's size through a ribbon or canvas control; such edits store an integer from 6 through 250.
...
- Future chapter or series schemas above version 16 are rejected without rewrite.
- Chapter load accepts older schema values, rebuilds typed objects, migrates legacy text alignment into free quads, normalizes current invariants, validates, and sets the in-memory schema to 16.
